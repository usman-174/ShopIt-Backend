...
import { default as catchAsyncError, default as CatchError } from "../middlewares/catchAsyncError";
...
export const profileUpdate = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    let profileData: {
      email?: string;
      name?: string;
...
export const AdminDeleteUser = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    const user = await User.findById(req.params.id);
    if (!user) return next(new errorHandler("User not found"));
    await user.remove();
